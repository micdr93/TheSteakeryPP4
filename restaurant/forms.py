...
class BookingForm(forms.ModelForm):
    ...
    additional_requests = forms.CharField(
        widget=forms.Textarea(
            attrs={'class': 'form-control form-center', 'rows': 3}
        ),
        required=False,
    ...
    class Meta:
        ...
        widgets = {
            'date': forms.DateInput(
                attrs={'type': 'date', 'class': 'form-control form-center'}
            ),
            'start_time': forms.TimeInput(
                attrs={'type': 'time', 'class': 'form-control form-center'}
            ),
            'num_guests': forms.NumberInput(
                attrs={'min': 1, 'class': 'form-control form-center'}
            ),
}

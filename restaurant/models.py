...
class Table(models.Model):
    ...
    def __str__(self):
        return (
            f"Table {self.table_number} - Max {self.max_capacity} num_guests"
        )
...
class Booking(models.Model):
    ...
    def clean(self):
        ...
        if (self.date - now).days > self.MAX_ADVANCE_BOOKING_DAYS:
            raise ValidationError(
                f"Bookings can only be made up to "
                f"{self.MAX_ADVANCE_BOOKING_DAYS} days in advance."
            )
        ...
        if self.start_time < opening_time or self.start_time >= closing_time:
            raise ValidationError(
                f"Reservations must be between "
                f"{opening_time.strftime('%I:%M %p')} and "
                f"{closing_time.strftime('%I:%M %p')} on "
        ...
        self.end_time = (
            ...
            ) + self.MAX_DINING_DURATION
        ).time()
        if self.end_time > closing_time:
            raise ValidationError(
                "The booking exceeds the restaurant's closing time."
            )

        # Table capacity validation
        if self.table and self.num_guests > self.table.max_capacity:
            raise ValidationError(
                f"Table {self.table.table_number} only accommodates "
                f"{self.table.max_capacity} num_guests."
            )
    ...
    def __str__(self):
        return (
            ...
            f"Table "
            f"{self.table.table_number if self.table else 'Unassigned'}, "
